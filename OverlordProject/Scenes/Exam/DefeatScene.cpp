...
#include "Prefabs/MarioGame/DefeatScreenHUD.h"
...
{
	m_ElapsedTime += m_SceneContext.pGameTime->GetElapsed();
	if (m_ElapsedTime > m_AnimationPlayTime)
	{
		m_Animator->Pause();
	}
}

void DefeatScene::OnSceneActivated()
{
	m_pMainMenuSound2D->setPosition(81300, FMOD_TIMEUNIT_MS);
	m_pMainMenuSound2D->setPaused(false);

	m_ElapsedTime = 0.f;
	m_Animator->SetAnimation(L"Die");
	m_Animator->Play();
}
void DefeatScene::OnSceneDeactivated()
{
	m_pMainMenuSound2D->setPaused(true);
}

void DefeatScene::LoadPlayer()
{
	...
	m_pMario->GetTransform()->Translate(-28.f, -10.f, -35.f);
	m_pMario->GetTransform()->Rotate(0.f, 0.f, 0.f);
	m_pMario->GetTransform()->Scale(0.6f);
	
	m_Animator = modelComp->GetAnimator();
}
void DefeatScene::LoadEnemy()
{
	// Model
	m_pGoomba = AddChild(new GameObject());
	auto mat = MaterialManager::Get()->CreateMaterial<DiffuseMaterial>();
	mat->SetDiffuseTexture(L"Textures/Goomba/Goomba.png");
	m_pGoomba->AddComponent(new ModelComponent(L"Meshes/Goomba.ovm"))->SetMaterial(mat);

	m_pGoomba->GetTransform()->Translate(-28.f, 3.f, -30.f);
	m_pGoomba->GetTransform()->Rotate(60.f, 0.f, 0.f);
	m_pGoomba->GetTransform()->Scale(1.5f);

}
void DefeatScene::LoadLevel()
{
	// Skybox
...
void DefeatScene::LoadSound()
{
	auto soundManager = SoundManager::Get();
	auto pFmodSystem = soundManager->GetSystem();

	FMOD::Sound* pSound{};
	pFmodSystem->createStream("Resources/Sounds/TitleTheme.mp3", FMOD_DEFAULT | FMOD_2D, nullptr, &pSound);
