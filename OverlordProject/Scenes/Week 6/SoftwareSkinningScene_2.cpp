...
void SoftwareSkinningScene_2::Update()
{
	...
	// Bone 0
	{
		const auto matrixWorld = XMLoadFloat4x4(&m_pBone0->GetTransform()->GetWorld());
		const auto matrixBindPose = XMLoadFloat4x4(&m_pBone0->GetBindPose());

		const auto boneTransform0 = XMMatrixMultiply(matrixBindPose, matrixWorld);

		for (size_t i = 0; i < 24; i++)
		{
			const auto vector = XMLoadFloat3(&m_SkinnedVertices[i].originalVertex.Position);
			const auto resultVec = XMVector3TransformCoord(vector, boneTransform0);
			XMStoreFloat3(&m_SkinnedVertices[i].transformedVertex.Position, resultVec);
		}
	}
	
	// Bone 1
	{
		const auto matrixWorld = XMLoadFloat4x4(&m_pBone1->GetTransform()->GetWorld());
		const auto matrixBindPose = XMLoadFloat4x4(&m_pBone1->GetBindPose());

		const auto boneTransform1 = XMMatrixMultiply(matrixBindPose, matrixWorld);

		for (size_t i = 24; i < m_SkinnedVertices.size(); i++)
		{
			const auto vector = XMLoadFloat3(&m_SkinnedVertices[i].originalVertex.Position);
			const auto resultVec = XMVector3TransformCoord(vector, boneTransform1);
			XMStoreFloat3(&m_SkinnedVertices[i].transformedVertex.Position, resultVec);
		}
	}
