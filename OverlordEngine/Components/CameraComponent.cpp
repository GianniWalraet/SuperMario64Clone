...
GameObject* CameraComponent::Pick(CollisionGroup ignoreGroups) const
{
	...
	float halfWidth = m_pScene->GetSceneContext().windowWidth * 0.5f;
	float halfHeight = m_pScene->GetSceneContext().windowHeight * 0.5f;
	XMFLOAT2 ndc{};
	ndc.x = (mousePos.x - halfWidth) / halfWidth;
	ndc.y = (halfHeight - mousePos.y) / halfHeight;

	auto nearPointVector = XMVector3TransformCoord( XMVectorSet(ndc.x, ndc.y, 0, 0), XMLoadFloat4x4(&m_ViewProjectionInverse));
	auto farPointVector = XMVector3TransformCoord( XMVectorSet(ndc.x, ndc.y, 1, 0), XMLoadFloat4x4(&m_ViewProjectionInverse));
	...
	PxVec3 rayPos = PxVec3{ nearPoint.x, nearPoint.y, nearPoint.z };
	PxVec3 rayDir = PxVec3({farPoint.x - nearPoint.x, farPoint.y - nearPoint.y, farPoint.z - nearPoint.z}).getNormalized();

	PxQueryFilterData filterData{};
	filterData.data.word0 = ~UINT(ignoreGroups);

	PxRaycastBuffer hit{};
	if (m_pScene->GetPhysxProxy()->Raycast(rayPos, rayDir, PX_MAX_F32, hit, PxHitFlag::eDEFAULT, filterData))
	{
		if (auto comp = static_cast<BaseComponent*>(hit.block.actor->userData))
		{
