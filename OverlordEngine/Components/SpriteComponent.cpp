...
void SpriteComponent::Draw(const SceneContext& /*sceneContext*/)
{
	...
	const auto& zRot = MathHelper::QuaternionToEuler(m_pGameObject->GetTransform()->GetRotation()).z;
	const auto& scale = m_pGameObject->GetTransform()->GetScale();

	SpriteRenderer::Get()->AppendSprite(m_pTexture, XMFLOAT2(pos.x, pos.y), m_Color, m_Pivot, XMFLOAT2(scale.x, scale.y), zRot, pos.z);
}
