...
void ShadowMapRenderer::Initialize()
{
	...
	int staticId = static_cast<int>(ShadowGeneratorType::Static);
	int skinnedId = static_cast<int>(ShadowGeneratorType::Skinned);
	m_GeneratorTechniqueContexts[staticId] = m_pShadowMapGenerator->GetTechniqueContext(staticId);
	m_GeneratorTechniqueContexts[skinnedId] = m_pShadowMapGenerator->GetTechniqueContext(skinnedId);
}
...
void ShadowMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	int idx = pMeshFilter->HasAnimations() ? static_cast<int>(ShadowGeneratorType::Skinned) : static_cast<int>(ShadowGeneratorType::Static);
	//2. Retrieve the corresponding TechniqueContext from m_GeneratorTechniqueContexts array (Static/Skinned)
	auto& techniqueContext = m_GeneratorTechniqueContexts[idx]; 
...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	//1. Making sure that the ShadowMap is unbound from the pipeline as ShaderResourceView (SRV) is important, because we cannot use the same 
	// resource as a ShaderResourceView (texture resource inside a shader) and a RenderTargetView (target everything is rendered too) at the 
	// same time. In case this happens, you'll see an error in the output of visual studio - warning you that a resource is still bound as 
	// a SRV and cannot be used as an RTV.
	...
	// your output because depending on your usage of ShaderResources, the actual slot the ShadowMap is using can be different, but you'll see a
	// warning pop-up with the correct slot ID in that case.
	...
	XMMATRIX matrixProj = XMMatrixOrthographicLH(viewWidth, viewHeight, nearZ, farZ);
	//	eyePosition: Position of the Direction Light (SceneContext::pLights > Retrieve Directional Light)
	auto& dLight = sceneContext.pLights->GetDirectionalLight();
	//	focusPosition: Calculate using the Direction Light position and direction
	FXMVECTOR eyePos = XMLoadFloat4( &dLight.position);
	FXMVECTOR lightDir = XMLoadFloat4(&dLight.direction);
	FXMVECTOR focusPos = eyePos + lightDir;

	//- Use the Projection & View Matrix to calculate the ViewProjection of this Light, store in m_LightVP
	XMMATRIX viewMat = XMMatrixLookAtLH(eyePos, focusPos, { 0,1,0,0 });
	XMStoreFloat4x4(&m_LightVP, viewMat * matrixProj);
...
void ShadowMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& meshBones)
{
	...
	const int staticID = static_cast<int>(ShadowGeneratorType::Static);
	const int skinnedID = static_cast<int>(ShadowGeneratorType::Skinned);
	//2. Retrieve the correct TechniqueContext for m_GeneratorTechniqueContexts
	auto& context = pMeshFilter->HasAnimations() ? m_GeneratorTechniqueContexts[skinnedID] : m_GeneratorTechniqueContexts[staticID];
	...
	if (pMeshFilter->HasAnimations()) 
		m_pShadowMapGenerator->SetVariable_MatrixArray(L"gBones", &meshBones.front()._11, static_cast<UINT>(meshBones.size()));
	...
	for (const auto& subMesh : pMeshFilter->GetMeshes())
	{
		//	- Set VertexBuffer
		const UINT offset = 0;
		const auto& vertexBufferData = pMeshFilter->GetVertexBufferData(context.inputLayoutID, subMesh.id);
		...
		const auto& tech = context.pTechnique;
