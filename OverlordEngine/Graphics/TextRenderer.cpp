...
{
	float w{}, h{};
	CalculateDimensions(pFont, textCache, w, h);
	...
	for(const wchar_t& character: textCache.text)
	{
		...
		VertexText vertex;
		vertex.position.x = textCache.position.x + float(totalAdvanceX + metric.offsetX)  - (w * 0.f);
		vertex.position.y = textCache.position.y + float(metric.offsetY) - (h * 2.f);
		vertex.position.z = .5f;
...
void TextRenderer::CalculateDimensions(SpriteFont* pFont, const TextCache& textCache, float& w, float& h)
{
	for (const wchar_t& character : textCache.text)
	{
		w += pFont->GetMetric(character).advanceX;
		h = pFont->GetMetric(character).height;
	}
