...
void SpriteRenderer::UpdateBuffer(const SceneContext& sceneContext)
{
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		bd.ByteWidth = sizeof(VertexSprite) * m_BufferSize;
		...
		HANDLE_ERROR(sceneContext.d3dContext.pDevice->CreateBuffer(&bd, nullptr, &m_pVertexBuffer));
	...
	if (m_pVertexBuffer)
	{
		...
		memcpy(mappedResource.pData, &m_Sprites[0], sizeof(VertexSprite) * m_BufferSize);
